...
#Method 6/7: Ridge with lambda.min/lambda.1se (R_min, R_1se)
eval_ridge_model = function(data, test_ids) {
  ...
  model_size[1] = sum(d^2/(d^2 + cv.out$lambda.min*ntrain))
  
  Ytest_pred = predict(cv.out, s = cv.out$lambda.1se, newx = X[test_ids, ])
  mspe[2] = mean((Y[test_ids] - Ytest_pred)^2)
  model_size[2] = sum(d^2/(d^2 + cv.out$lambda.1se*ntrain))
 
  list(mspe=mspe, model_size = model_size) 
}

#Method 8/9: Lasso using lambda.min/lambda.1se (L_min, L_1se)
eval_lasso_model = function(data, test_ids) {
  mspe = rep(0, 2)
  model_size = rep(0, 2)
...
eval_functions = c(eval_full_model, eval_aic_f_model, eval_aic_b_model,       
                  eval_bic_f_model, eval_bic_b_model, eval_ridge_model, 
                  eval_lasso_model, eval_lasso_refit_model)
...
```
Computation Time (Note: R.min with R.1se and L.min with L.1se are calculated using one function, thus we make their computation time the same)
```{r, echo=FALSE}
...
```{r, echo=FALSE, fig.width=10, fig.height=4}
eval_functions = c(eval_ridge_model, eval_lasso_model, eval_lasso_refit_model)
