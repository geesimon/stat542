...
- Missing value: 'Garage_Yr_Blt' has some missing values, 'Year_Built' is used to fill the value. Note: 'Garage_Yr_Blt' is removed due to low importance, I still leave this step for generailize the processing pipeline.
...
make_bin = function(train_data, test_data){
  # cond_levels = c("Poor", "Average", "Above_Average", "Good", "Very_Good", "Excellent")
  # cond_map=c("Poor", "Poor", "Poor", "Poor", "Average", "Above_Average",
  #            "Good", "Very_Good", "Excellent", "Excellent")
  # 
  ...
  train_data$Year_Built[train_data$Year_Built > 2005] = 2006
  train_data$Year_Built[train_data$Year_Built >1985 & train_data$Year_Built <= 2005] = 1986
  train_data$Year_Built[train_data$Year_Built > 1973 & train_data$Year_Built <= 1985] = 1974
  train_data$Year_Built[train_data$Year_Built <= 1973] = 1973
  train_data$Year_Built = as.ordered(train_data$Year_Built)
  
  test_data$Year_Built[test_data$Year_Built > 2005] = 2006
  test_data$Year_Built[test_data$Year_Built >1985 & test_data$Year_Built <= 2005] = 1986
  test_data$Year_Built[test_data$Year_Built > 1973 & test_data$Year_Built <= 1985] = 1974
  test_data$Year_Built[test_data$Year_Built <= 1973] = 1973
  test_data$Year_Built = as.ordered(test_data$Year_Built)
...
preprocess_data = function(train_data, test_data){
  ...
  #r = make_bin(r$train_data, r$test_data)
  ...
  if(hasName(r$test_data, "Sale_Price")){
    ...
    r$test_data = r$test_data[, !colnames(r$test_data) %in% c('Sale_Price')] #Remove "Sale_Price" column  
  } else {
    true_test_y = NULL
  }
...
test_all = function (all_data, all_test_pid, reg_func) {
  rmse = matrix(0, dim(all_test_pid)[2], length(reg_func))
  colnames(rmse) = names(reg_func)
  
  for (i in c(3)){
  #for (i in 1:dim(all_test_pid)[2]){
...
rmse = test_all(all_data, Project1_test_id, regression_functions)
```
...
Computing time: `r (proc.time() - start_time)[3]` seconds

```{r, include=FALSE}
start_time = proc.time()
if(!exists("true_test_value")) true_test_value = NULL
train_predict(train_data, test_data, lasso_predict, "mysubmission1.txt", true_test_value)
```
