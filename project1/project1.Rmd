...
```{r}
mypackages = c("randomForest","gbm","glmnet","psych")   # required packages
...
process_categorical = function(train_data, test_data){
  for (name in names(CategoricalLevels)) {
    ...
    if(CategoricalLevels[[name]][n] != "Misc") {
      ordered_levels = CategoricalLevels[[name]][CategoricalLevels[[name]] %in% unique(train_data[,name])]
      
      train_data[,name] = ordered(train_data[,name], levels = ordered_levels)
      
      test_data[,name] = ordered(test_data[,name], levels = ordered_levels)
