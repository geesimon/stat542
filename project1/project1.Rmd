...
process_numeric = function(train_data, test_data) {
  train_data$Year_Sold = factor(train_data$Year_Sold)
  train_data$Mo_Sold = factor(train_data$Mo_Sold)
  test_data$Year_Sold = factor(test_data$Year_Sold)
  test_data$Mo_Sold = factor(test_data$Mo_Sold)
...
process_output = function (train_data, test_data) {
  ...
  train_data = Winsorizing(train_data, "Sale_Price", 0.01)
...
for (i in 1:length(rf_rmse)){
  ...
  rfModel = randomForest(Sale_Price ~ . - PID, data = r$train_data, importance = T, ntree=500); 
  #(proc.time() - start_time)[3]
  yhat_test = predict(rfModel, r$test_data)
  rf_rmse[i] = get_RMSE(yhat_test, r$test_data$Sale_Price)
  print(rf_rmse[i])
}
...
for (i in 1:length(gbm_rmse)){
  ...
  gbmModel = gbm(Sale_Price ~ ., data = r$train_data, distribution = "gaussian", n.trees = 3000,
