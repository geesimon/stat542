  ...
  for (name in names(CategoricalLevels)) {
    ...
    if(CategoricalLevels[[name]][n] != "Misc") {
      ...
      if(sum(!test_levels %in% train_levels) > 0) {
        ...
        cat("Undefined test cate:", name, "\n")
        cat("Undefined test value:", test_levels[!test_levels %in% train_levels], "\n")
        cat("Levels:", train_levels, "\n")
...
gbm_predict = function(train_data, test_data) {
  gbmModel = gbm(Sale_Price ~ ., data = train_data, distribution = "gaussian", n.trees = 5000,
               shrinkage = 0.05, interaction.depth = 6, bag.fraction = 0.5, cv.folds = 5, verbose = FALSE)
...
xgb_predict = function(train_data, test_data) {
  ...
  xgb_model = xgboost(data = X_train, label=Y_train, max_depth = 3, 
                      eta = 0.05, nthread = 4, nrounds = 3000, verbose = FALSE)
...
#regression_functions = c(rf_predict, gbm_predict, lasso_predict, xgb_predict)
