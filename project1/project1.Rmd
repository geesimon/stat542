...
```{r}
mypackages = c("randomForest","gbm","glmnet","psych", "xgboost", "caret")   # required packages
...
process_numeric = function(train_data, test_data) {
  ...
  for (i in 2:dim(train_data)[2]){
    if(colnames(train_data)[i] %in% c("Sale_Price", names(CategoricalLevels)) ||
...
mylasso_myfit = function(X, y, lam, beta = NULL)
{
  ...
  cat("rss_chnaged",rss_changed, "step", step, "\n")
...
cv.mylasso_fit = function(train_data, cv_fold = 5) {
  ...
  all_test_ids = matrix(0, cv_fold, nrow(X_train) / cv_fold)
  for (i in 1:cv_fold){
      all_test_ids[i,] = sample(1:nrow(X_train), nrow(X_train) / cv_fold)
  }
  ...
  for (lambda_i in 1:length(all_lambda)) {
    for (i in 1:cv_fold){
      ...
      beta = mylasso_myfit(X_train, Y_train, all_lambda[lambda_i], cv_beta[lambda_i,])
...
```
```{r, eval=FALSE, include=FALSE}
get_RMSE(mylasso_predict(train_data, test_data), r$test_data$Sale_Price)
```
...
lasso_predict = function(train_data, test_data) {
  X_train = train_data[, colnames(train_data) != 'Sale_Price']
  X_train = model.matrix(~., X_train)[, -1]
  #X_train = predict(dummyVars(~., data = X_train), newdata = X_train)
  ...
  X_test = test_data[, colnames(test_data) != 'Sale_Price']
  X_test = model.matrix(~. -PID, X_test)[, -1]
  #X_test = predict(dummyVars(~. -PID, data = X_test), newdata = X_test)
...
xgb_predict = function(train_data, test_data) {
  ...
  #                     eta = 0.05, nthread = 4, nrounds = 5000, verbose = FALSE)
  # 
  # X_test = test_data[, colnames(test_data) != 'Sale_Price']
  # X_test = model.matrix(~. - PID, X_test)[,-1]
...
#regression_functions = c(xgb_predict)
#regression_functions = c(gbm_predict, xgb_predict)
...
rmse = matrix(0, dim(Project1_test_id)[2], length(regression_functions))
colnames(rmse) = names(regression_functions)
...
for (i in 1:10){
  ...
  train_data = r$train_data[, !colnames(r$train_data) %in% c('PID')] #Remove 'PID' column
  test_data = r$test_data[, !colnames(r$test_data) %in% c('Sale_Price')] #Remove "Sale_Price" column
  ...
  for (f in 1:length(regression_functions)) {
    yhat_test = regression_functions[[f]](train_data, test_data)
...
Y_test = r$test_data$Sale_Price

beta = mylasso_myfit(X_train, Y_train, 10)

Ytest_pred = mylasso_mypredict(X_test, beta)
get_RMSE(Ytest_pred, Y_test)
```
