...
rf_predict = function(train_data, test_data) {
  ...
  cat("training rmse:",sqrt(rfModel$mse[length(rfModel$mse)]), "\n")
...
xgb_predict = function(train_data, test_data) {
  ...
  xgb_model = xgboost(data = X_train, label=Y_train, max_depth = 6,
                      eta = 0.01, nthread = 4, nrounds = 10000, verbose = FALSE)
  ...
  # fitControl = trainControl(method = "repeatedcv", number = 5)
  # xgbGrid =  expand.grid(max_depth = c(6, 9),
  #                         nrounds = c(2000,5000,10000),
  #                         eta = c(0.1, 0.01),
